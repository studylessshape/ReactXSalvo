...
pub async fn add_menu(
    ...
    if menu::Entity::find()
        .filter(menu::Column::Key.eq(key.clone()))
        .one(&conn)
        .await?
        .is_some()
    {
...
pub async fn delete_menu(
    _token: CookieParam<String, true>,
    data: JsonBody<Id<Uuid>>,
) -> JsonResult<()> {
    let Id { id } = data.into_inner();
    let conn = db::conn().await?;
...
pub async fn get_menu(_token: CookieParam<String, true>) -> JsonResult<Vec<MenuModel>> {
    let conn = db::conn().await?;
    let res = menu::Entity::find()
        .into_partial_model::<MenuModel>()
        .all(&conn)
        .await?;
    json_ok(res)
}
